...
import { Beaker, AlertTriangle, FileText, BookOpen, ExternalLink } from "lucide-react"

export default function ChemicalManagementLawPage() {
  const [lawInfo, setLawInfo] = useState<LawData | null>(null)
  const [articles, setArticles] = useState<LawArticle[]>([])
  const [selectedArticle, setSelectedArticle] = useState<string | null>(null)

  useEffect(() => {
    const info = getLawInfo("chemical-management")
    const lawArticles = getArticlesByLaw("chemical-management")
      ...
      <div className="bg-gradient-to-r from-green-50 to-green-100 border-b border-green-200">
        <div className="max-w-6xl mx-auto px-6 py-8">
          <div className="flex items-start gap-6">
            <div className="p-4 bg-green-100 rounded-notion-lg">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-sm text-green-700">{lawInfo.lawNumber}</span>
              </div>
              <h1 className="text-3xl font-bold text-green-900 mb-3">{lawInfo.lawName}</h1>
              {lawInfo.lawNameEng && (
                <p className="text-lg text-green-700 mb-4">{lawInfo.lawNameEng}</p>
              )}
              <p className="text-green-800 leading-relaxed max-w-4xl">{lawInfo.summary}</p>
              
              <div className="flex items-center gap-6 mt-4 text-sm text-green-700">
              ...
              {articles.map((article) => (
                ...
                >
                  <div className="flex items-start gap-4 mb-4">
                    <div className="p-2 bg-green-100 rounded-notion-md">
